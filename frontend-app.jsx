...
const KyivBookstoresApp = () => {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        resetForm();
        alert('Книгарню успішно додано!');
      } else {
        const error = await response.json();
        alert('Помилка: ' + (error.error || 'Не вдалося додати книгарню'));
      }
    } catch (err) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (response.ok) {
        await fetchBookstores();
        resetForm();
        alert('Книгарню успішно оновлено!');
      } else {
        alert('Не вдалося оновити книгарню');
      }
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (response.ok) {
        await fetchBookstores();
        alert('Книгарню видалено!');
      } else {
        alert('Не вдалося видалити книгарню');
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50">
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white p-6 shadow-lg">
        ...
        <p className="text-sm opacity-90 mb-4">Підключено до Microsoft SQL Server 2022</p>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <h3 className="text-lg font-semibold text-gray-800">
                Знайдено: {bookstores.length}
              </h3>
              {bookstores.length === 0 ? (
                <div className="bg-white rounded-xl shadow-md p-8 text-center text-gray-500">
                  Книгарні не знайдено
                </div>
              ) : (
